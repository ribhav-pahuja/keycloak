...
On a FIPS enabled platform with FIPS enabled OpenJDK 25, you can run this to test against a Keycloak server with FIPS 140-2 integration enabled.
...
NOTE 2: Example of the server startup log, running in an environment (both -> JVM and operating system/container), where FIPS is enabled:
```
