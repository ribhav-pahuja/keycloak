...
benefits from the use of virtual threads which are available when at least four cores are available for {project_name}.
This reduces the memory usage and removes the need to configure thread pool sizes.
For best performance, use OpenJDK {jdk_version_latest} or later.
