...
NOTE: Thread pool metrics are not available with virtual threads. Virtual threads are enabled by default when running with OpenJDK 25.
...
JGroups delays the acknowledgment of delivered messages, to reduce the amount of acknowledge messages in transit.
This choice has a side effect: the sender may eagerly retransmit message(s) if the acknowledges are delayed for too long.
An increase in the retransmission metric value does not immediately mean an unhealthy cluster.
...
TIP: A healthy cluster typically shows a value of zero or very low values for all metrics.
Occasional retransmissions may occur due to timing issues between the sender and the receiver.
